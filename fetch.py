...
def await_DOM(driver: Edge):
    return driver.find_element(By.TAG_NAME, "div")


def await_Input(driver: Edge):
    return driver.find_element(By.TAG_NAME, "div")
...
def retrieve(url: str):
    ...
    if not driver:
        ...
        cookies = setup.get_cookies(
            "./cookies/www.cloudskillsboost.google_07-11-2022.json")

        # If the correct_json script has not been run this step will fail
        setup.add_cookies(setup.transform_cookies(cookies), driver)

        # WebDriverWait(driver, timeout=4).until(await_Input)
        time.sleep(3)
        driver.refresh()
    ...
    # WebDriverWait(driver, timeout=4).until(await_DOM)
    time.sleep(3)
